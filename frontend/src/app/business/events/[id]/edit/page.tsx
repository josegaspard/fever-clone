...
export default function EditBusinessEventPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [event, c, cat, pol] = await Promise.all([
        ...
        setForm({
          ...
          refundPolicyId: event.refundPolicyId || '',
          metaTitle: (event as unknown as Record<string, unknown>).metaTitle as string || '',
          metaDescription: (event as unknown as Record<string, unknown>).metaDescription as string || '',
        });
      } catch {
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'var(--overlay)' }}>
          ...
          >
            ...
            <h3 className="text-lg font-bold text-center mb-2" style={{ color: 'var(--fg)' }}>
              Eliminar evento
            </h3>
            <p className="text-sm text-center mb-6" style={{ color: 'var(--text-secondary)' }}>
              Esta accion es irreversible. Se eliminaran todos los datos asociados a este evento.
            </p>
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2.5 text-sm font-bold rounded-xl transition disabled:opacity-50"
      ...
      <div className="flex flex-col xl:flex-row gap-8">
        ...
        <form onSubmit={handleSubmit} className="flex-1 space-y-6">
          ...
          <div className="rounded-2xl border p-6 space-y-5" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
            ...
            <div>
              <label className={labelClass} style={{ color: 'var(--text-secondary)' }}>Meta titulo</label>
              ...
              <span className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>{(form.metaTitle || form.title).length}/60 chars</span>
            </div>
            <div>
              <label className={labelClass} style={{ color: 'var(--text-secondary)' }}>Meta descripcion</label>
              <textarea value={form.metaDescription} onChange={(e) => updateField('metaDescription', e.target.value)} placeholder={form.shortDescription || 'Descripcion para buscadores'} maxLength={170} rows={3} className={inputClass} style={{ resize: 'vertical' }} />
              <span className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>{(form.metaDescription || form.shortDescription || '').length}/160 chars</span>
            </div>
            <div className="rounded-xl p-4" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
              ...
              <p className="text-xs truncate" style={{ color: '#006621' }}>ctxplorer.com/events/{form.slug}</p>
